...
public interface TransactionRepository extends CrudRepository<Transaction, Long> {
	 ...
	 */
	@Query(nativeQuery=true, value="SELECT SUM(amount) FROM Transaction WHERE Month(date) = :month AND Year(date) = :year ;")
	 ...
	 */
	@Query(nativeQuery=true, value="SELECT * FROM Transaction WHERE Name LIKE '%' + :name + '%' OR Description LIKE '%' + :name: + '%' ;")
	 ...
	 */
	@Query(nativeQuery=true, value="SELECT * FROM Transaction WHERE Month(date) = :month AND Year(date) = :year ;")
	 ...
	 */
	@Query(nativeQuery=true, value="SELECT * FROM Transaction WHERE amount > :amount AND Month(date) = :month AND Year(date) = :year;")
	...
	/*
	 * Finds all transactions related to a budget ID
	 * @param budgetId: The primary key of the budget
	 * @return: List of related transactions
	 */
	@Query(nativeQuery=true, value="SELECT * FROM Transaction WHERE BudgetId = :budgetId")
	public List<Transaction> getTransactionsByBudgetID(@Param("budgetId") int budgetId);
}
